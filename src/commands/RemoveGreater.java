...
/**
 * The RemoveGreater class implements the {@link ICommand} interface and provides functionality
 * to remove all flats from the collection whose area is greater than the specified area
 * or a given flat.
 */
public class RemoveGreater implements ICommand{
    ...
    /**
     * Constructs a new RemoveGreater command.
     *
     ...
     */
    public RemoveGreater(int id, String arguments, CommandControl commandControl, CollectionControl collectionControl) {
        super();
    ...
    /**
     * Executes the command to remove flats from the collection whose area is greater than
     * the specified flat or area. If the area or flat details are invalid, an error response
     * is returned.
     *
    ...
    private Response executeCommand(Request request) {
        if (request instanceof ServerRequest) {
            ...
            long count = collectionControl.removeGreater(flat, req.getUser().getName());
        ...
        if (request.getType() == RequestType.EXECUTE_COMMAND) {
            ...
            if (r.getType() == ResponseType.SUCCESS) {
                Flat flat = builder.build();
                long count = collectionControl.removeGreater(flat, "ADMIN");
                return new Response(getId(), ResponseType.SUCCESS, PrintType.LINE, "Удалены все квартиры, площадью больше " + request.getContent() + ": " + count + " штук");
            }
        ...
        if (resp.getType() == ResponseType.SUCCESS) {
            if (prompt != null) return new Response(getId(), ResponseType.REQUEST_VALUES, PrintType.NONE, resp.getContent(), prompt);
                ...
                Flat flat = builder.build();
                long count = collectionControl.removeGreater(flat, "ADMIN");
                return new Response(getId(), ResponseType.SUCCESS, PrintType.LINE, "Удалены все квартиры большие данной: " + count + " штук");
            }
        }
    ...
    /**
     * The Factory class provides methods to create instances of the RemoveGreater command
     * and describe its functionality.
     */
    public static class Factory implements ICommandFactory {
        
        /**
         * Creates a new instance of the RemoveGreater command.
         *
         * @param id               The unique identifier for the command.
         * @param arguments        The initial arguments specifying the area or flat (required).
         * @param commandControl   The {@link CommandControl} instance for managing commands.
         * @param collectionControl The {@link CollectionControl} instance for managing the collection.
         * @return A new instance of the RemoveGreater command.
         */
        @Override
        public ICommand create(int id, String arguments, CommandControl commandControl, CollectionControl collectionControl, Request request) {
            return new RemoveGreater(id, arguments, commandControl, collectionControl);
        }

        /**
         * Returns a brief description of the RemoveGreater command.
         *
         * @return A short description of the command functionality.
         */
        @Override
        public String getDescription() {
            return "remove_greater area / {flat}: удалить из коллекции все квартиры, большие заданной по площади";
        }

        /**
         * Returns an extended description of the RemoveGreater command, providing details
        ...
        public String getExtendedDescription() {
            return "remove_greater {flat}: удалить из коллекции все квартиры, большие заданной по площади. Необходимые значения для flat будут запрошены в последующих строчках.\nremove_greater area: удалить из коллекции все квартиры, площадь которых, больше area.";
        }
    }
}
